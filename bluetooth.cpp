...
int getVirtualJoystickPositionX(QSerialPort *port){
    int x = 15;
...
int getVirtualJoystickPositionY(QSerialPort *port){
    int y = 15;
    const char output[10] = "AT+WGVP\r\n";
    port->write(output);
    port->flush();
...
int getMaximumSpeed(QSerialPort *port){
    int speed = 15;
...
int getProfile(QSerialPort *port){
    int profile = 9;
...
int getBatteryLvl(QSerialPort *port){
    int battery = 10;
    const char output[10] = "AT+WGMD\r\n";
    port->write(output);
    port->flush();
